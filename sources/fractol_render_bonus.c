...
/*   Updated: 2025/01/29 08:56:22 by mpiantan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * palette[1] = shades of purple-red
 * palette[2] = shades of purple-pink
 * palette[3] = shades of red-orange
 * palette[4] = shades of yellow-green
 * palette[5] = shades of green
 */
...
int	get_color(int iteration, t_fractal *fractal, const t_palette *palette)
{
	...
	if (fractal->type == 2)
		...
		tr = (double)iteration / (MAX_ITERATION);
...
int	calculate_fractal_iteration(t_fractal *fractal, int x, int y)
{
	...
	else if (fractal->type == 2)
	{
		...
		return (burning_ship_set(c));
	else if (fractal->type == 4)
		return (tricorn_set(c));
	else if (fractal->type == 5)
		return (multibrot_set(c, fractal->degree));
	return (0);
}
